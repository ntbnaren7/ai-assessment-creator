        ...
        questions: [
          {
            ...
            difficulty: "Moderate",
            marks: 13,
            questionType: "Numerical Problems",
      ...
      updateQuestionTypeRow: (id, updated) =>
        set(
          (state) => {
            ...
            // Calculate current totals excluding the row being updated
            const otherRows = state.form.questionTypeRows.filter((r) => r.id !== id);
            ...
            // Clamp numberOfQuestions so total never exceeds MAX_TOTAL_QUESTIONS
            let clampedQuestions = Math.max(1, updated.numberOfQuestions);
            const maxQuestionsRemaining = MAX_TOTAL_QUESTIONS - otherQuestions;
            clampedQuestions = Math.min(clampedQuestions, Math.max(1, maxQuestionsRemaining));

            // Clamp marks so total never exceeds MAX_TOTAL_MARKS
            let clampedMarks = Math.max(1, updated.marks);
            const maxMarksRemaining = MAX_TOTAL_MARKS - otherMarks;
            const maxMarksPerQuestion = Math.max(1, Math.floor(maxMarksRemaining / clampedQuestions));
            clampedMarks = Math.min(clampedMarks, maxMarksPerQuestion);
